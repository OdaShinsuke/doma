...
public class MssqlPagingTransformerTest extends TestCase {
    ...
    public void testOffsetLimit_for() throws Exception {
        String expected = "select emp.id from emp order by emp.id offset 5 rows fetch next 10 rows only for xml auto";
        MssqlPagingTransformer transformer = new MssqlPagingTransformer(
                5, 10);
        SqlParser parser = new SqlParser(
                "select emp.id from emp order by emp.id for xml auto");
        SqlNode sqlNode = transformer.transform(parser.parse());
        NodePreparedSqlBuilder sqlBuilder = new NodePreparedSqlBuilder(
                new MockConfig(), SqlKind.SELECT, "dummyPath");
        PreparedSql sql = sqlBuilder.build(sqlNode, Function.identity());
    ...
    public void testOffsetLimit_forOption() throws Exception {
        String expected = "select emp.id from emp order by emp.id offset 5 rows fetch next 10 rows only for xml auto option (maxrecursion 0)";
        MssqlPagingTransformer transformer = new MssqlPagingTransformer(
                5, 10);
        SqlParser parser = new SqlParser(
                "select emp.id from emp order by emp.id for xml auto option (maxrecursion 0)");
        SqlNode sqlNode = transformer.transform(parser.parse());
        NodePreparedSqlBuilder sqlBuilder = new NodePreparedSqlBuilder(
                new MockConfig(), SqlKind.SELECT, "dummyPath");
        PreparedSql sql = sqlBuilder.build(sqlNode, Function.identity());
    ...
    public void testOffsetOnly_for() throws Exception {
        String expected = "select emp.id from emp order by emp.id offset 5 rows for xml auto";
        MssqlPagingTransformer transformer = new MssqlPagingTransformer(
                5, -1);
        SqlParser parser = new SqlParser(
                "select emp.id from emp order by emp.id for xml auto");
    ...
    public void testOffsetOnly_forOption() throws Exception {
        String expected = "select emp.id from emp order by emp.id offset 5 rows for xml auto option (maxrecursion 0)";
        MssqlPagingTransformer transformer = new MssqlPagingTransformer(
                5, -1);
        SqlParser parser = new SqlParser(
                "select emp.id from emp order by emp.id for xml auto option (maxrecursion 0)");
    ...
    public void testLimitOnly_for() throws Exception {
        String expected = "select emp.id from emp order by emp.id offset 0 rows fetch next 10 rows only for xml auto";
        MssqlPagingTransformer transformer = new MssqlPagingTransformer(
                -1, 10);
        SqlParser parser = new SqlParser(
                "select emp.id from emp order by emp.id for xml auto");
    ...
    public void testLimitOnly_forOption() throws Exception {
        String expected = "select emp.id from emp order by emp.id offset 0 rows fetch next 10 rows only for xml auto option (maxrecursion 0)";
        MssqlPagingTransformer transformer = new MssqlPagingTransformer(
                -1, 10);
        SqlParser parser = new SqlParser(
                "select emp.id from emp order by emp.id for xml auto option (maxrecursion 0)");
